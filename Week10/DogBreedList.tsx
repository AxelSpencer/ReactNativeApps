...
const DogBreedList: React.FC<{ route: any }> = ({ route }) => {
  ...
  const fetchData = () => {
    setRefreshing(true);
    axios
      .get(`https://dog.ceo/api/breed/${breed.toLowerCase()}/images/random/30`)
      .then((response) => {
  ...
  const renderItem = ({ item, index }: { item: string; index: number }) => (
    <TouchableOpacity
      onPress={() => {
        navigation.navigate("DogModal", { url: item, breed: breed });
      }}
      style={[
